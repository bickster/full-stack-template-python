...
def test_auth_endpoints():
    ...
    for method, path, description in expected_endpoints:
        ...
        if found:
            route = next(r for r in auth_routes if r['method'] == method and r['path'] == path)
...
def test_user_endpoints():
    ...
    for method, path, description in expected_endpoints:
        ...
        if found:
            route = next(r for r in user_routes if r['method'] == method and r['path'] == path)
            print(f"         Function: {route['function']}")
            if route['response_model']:
                print(f"         Response: {route['response_model']}")
...
def test_endpoint_security():
    ...
    print(f"✅ Rate limiting configured on: {', '.join(rate_limit_endpoints) if rate_limit_endpoints else 'None detected'}")
